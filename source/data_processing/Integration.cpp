...
std::vector<IntegrationVector> Integration::dormand_prince(IntegrationVector initial_condition, Date* start, Date* end, double step, std::map<std::string, std::vector<IntegrationVector>> planets)
{
    ...
    {
        k1 = derivate_function(t, new_condition, planets);
        k2 = derivate_function(t + c2 * step, new_condition + step * (a21 * k1), planets);
        k3 = derivate_function(t + c3 * step, new_condition + step * (a31 * k1 + a32 * k2), planets);
        k4 = derivate_function(t + c4 * step, new_condition + step * (a41 * k1 + a42 * k2 + a43 * k3), planets);
        k5 = derivate_function(t + c5 * step, new_condition + step * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4), planets);
        k6 = derivate_function(t + step, new_condition + step * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5), planets);
        k7 = derivate_function(t + step, new_condition + step * (a71 * k1 + a72 * k2 + a73 * k3 + a74 * k4 + a75 * k5 + a76 * k6), planets);
