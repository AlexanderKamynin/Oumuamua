...
void LightCorrector::light_time_correction(std::vector<Observation>* observations, std::map<std::string, Observatory>* observatory, std::vector<IntegrationVector>* model_measure, std::vector<IntegrationVector>* sun_info)
{
	...
	*/
	...
	for (int i = 0; i < model_measure->size(); i++)
	{
		...
		//std::cout << std::setprecision(9) << "After light time correct " << observatory_position.get_alpha() << ";" << observatory_position.get_beta() << ";" << observatory_position.get_gamma() << std::endl;
		...
		//std::cout << std::setprecision(9) << "After gravitational deflection " << observatory_position.get_alpha() << ";" << observatory_position.get_beta() << ";" << observatory_position.get_gamma() << std::endl;
		// abberation
		this->aberration(&body_position, &observatory_position, &sun_position);
		//std::cout << std::setprecision(9) << "After aberration " << observatory_position.get_alpha() << ";" << observatory_position.get_beta() << ";" << observatory_position.get_gamma() << std::endl << std::endl;
